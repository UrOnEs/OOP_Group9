...
int main() {
    ...
    sf::Font menuFont;
    if (!menuFont.loadFromFile("assets/UI/SuperPixel.ttf")) {
        std::cerr << "HATA: asset bulunamadý" << std::endl;
    }

    // --- 2. Button Texture Yükleme ---
    sf::Texture btnTexture;
    if (!btnTexture.loadFromFile("assets/UI/button2.png")) {
    ...
    else if (bgTexture.loadFromFile("assets/UI/background.jpg")) {
        hasBg = true;
        bgSprite.setTexture(bgTexture);
        sf::Vector2u texSize = bgTexture.getSize();
        bgSprite.setScale((float)WIN_W / texSize.x, (float)WIN_H / texSize.y);
    ...
    if (lobbySelectionBgTexture.loadFromFile("assets/UI/lobbySelection.png")) {
    ...
    if (lobbyBgTexture.loadFromFile("assets/UI/lobbyBackground.jpg")) {
        hasLobbyBg = true;
    ...
    if (gameNameTexture.loadFromFile("assets/UI/Empire of Ages1.png")) {
