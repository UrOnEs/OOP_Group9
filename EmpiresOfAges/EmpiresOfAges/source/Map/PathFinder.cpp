...
// Helper: Line of Sight check (Bresenham-like Raycast)
// Essential for "Pixel perfect" movement feeling.
bool isLineClear(Point start, Point end, const std::vector<int>& mapData, int width, int height) {
    ...
    while (true) {
        ...
        // Prevents units from walking through diagonal gaps between walls.
        if (stepX && stepY) {
            int checkX = x0 - sx;
            int checkY = y0 - sy;
...
// Removes unnecessary zig-zags if a straight line is clear.
...
Point PathFinder::findBestTargetTile(Point startNode, Point targetCenter, int targetSizeInTiles, const std::vector<int>& mapData, int width, int height, const std::set<Point>& reservedTiles) {
    ...
    // 1. Define the area around the building
    ...
    // 2. Collect all empty tiles around the target's perimeter
    for (int x = rectX - 1; x <= rectX + rectW; ++x) {
        for (int y = rectY - 1; y <= rectY + rectH; ++y) {

            // Skip the building interior
    ...
    // If completely surrounded, fallback to BFS for nearest free tile
    if (candidates.empty()) {
        return findClosestFreeTile(targetCenter, mapData, width, height, reservedTiles);
    }

    // 3. Sort candidates by distance to the unit
    std::sort(candidates.begin(), candidates.end(), [startNode](Point a, Point b) {
        return heuristic(startNode, a) < heuristic(startNode, b);
        });

    // --- CRITICAL CHANGE: Line of Sight Check ---
    // Instead of blindly picking the closest, pick the first one with a clear path.
    int checkLimit = 0;
    for (const auto& p : candidates) {
        if (isLineClear(startNode, p, mapData, width, height)) {
            return p; // Found! Accessible and close.
        }
    ...
    // Fallback: Just pick the closest if no direct line is found.
    return candidates[0];
}
